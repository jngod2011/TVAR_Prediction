...
Par le lemme \ref{lem:Gamma_non_inv}, le fait que $\Gamma_{p+1}$ soit non inversible implique qu'il existe $t\in \zset$ tel que $X_t \in \calH^X_{t-1,p}$. \\
On sait que $\forall k \in \iseg{1,p}, \calH^X_{t-1,k} \subset \calH^X_{t-1,k+1} \subset \cdots \subset \calH^X_{t-1,p}$. Il existe donc $p_0\in \iseg{1,p}$ tel que 
$$
X_t \in \calH^X_{t-1,p_0} \mbox{ et } X_t \notin \calH^X_{t-1,p_0-1}
$$
(avec la convention $\calH^X_{t-1,0} = \emptyset$). On a alors
$$
X_t = \proj(X_t|\calH^X_{t-1,p_0}) = \sum_{k=1}^{p_0} \theta_{k,p_0} X_{t-k} = \sum_{k=1}^{p_0-1} \theta_{k,p_0} X_{t-k} + \kappa_{p_0} X_{t-p_0}
$$
avec $\kappa_{p_0} \neq 0$ car sinon on aurait $X_t \in \calH^X_{t-1,p_0-1}$. Donc 
$$
X_t - \proj(X_t|\calH^X_{t-1,p_0-1}) = X_t - \sum_{k=1}^{p_0-1} \theta_{k,p_0} X_{t-k} - \kappa_{p_0} \proj(X_{t-p_0}|\calH^X_{t-1,p_0-1}) = \kappa_{p_0} \left[ X_{t-p_0} - \proj(X_{t-p_0}|\calH^X_{t-1,p_0-1}) \right]
$$
